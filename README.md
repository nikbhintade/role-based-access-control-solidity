...
If Node.js isn't already installed, you can download it from the [official website](https://nodejs.org/) and install it by following the instructions. Once Node.js is installed, you can proceed with the tutorial.

We also need to install in the browser a wallet that works with Celo, like MetaMask or Alfajores.

## Project Setup

In this tutorial, we are going to write and test smart contracts along with a web app. For the smart contract, we are going to use Hardhat, and for the web app, React. Let's get started on both.
...
In this tutorial, we will build a dapp that will allow users with certain roles to create an entry in the contract. The role will have a role admin that controls that role and a default admin that can control all the roles.

The role admin has the ability to grant and revoke roles. The default administrator will have control over everything related to roles, like changing the administrator's role and granting and revoking roles for any user.
...
We inherit from the **`AccessControl`** contract to implement RBAC functionality in our smart contract:
...
After renouncing the role, the function emits a **`RoleGranted`** event.
...
As we have written and tested our smart contract, we can proceed with building the web app. To create the web app, we need the contract ABI. We can generate the ABI by running the **`test`** command. Copy the **`RBAC.json`** file from the **`artifacts/contracts/RBAC.sol`** directory to **`frontend/src`**.
...
```bash
npx hardhat run --network alfajores run scripts/deploy.js
```
...
Command should open a tab on your browser. If it doesn’t open a tab automatically, you can check web app on [http://localhost:3000/](http://localhost:3000/) on your pc.
...
In conclusion, we have successfully developed a RBAC (Role-Based Access Control) dapp using OpenZeppelin library which allows us to control access to certain functions within the contract. Throughout this tutorial, we learned about important concepts like role admins and default admins, which enabled us to effectively manage the roles in the contract. We also gained knowledge on how to test smart contracts with the help of Hardhat framework and Chai testing library. Additionally, we learned how to test events with the help of regular expressions, which proved to be a valuable tool in testing smart contracts.

Moreover, we also built a web app to interact with our RBAC smart contract. We focused on the logic and functions of the web app, rather than its structure and styling. The web app enables users to connect their wallets, create entries, and manage roles effectively through a user-friendly interface.
...
Congratulations on completing this tutorial! You've made it to the end, which is no small feat. Share your amazing work on Celo Discord so that we can all marvel at your brilliance!
...
Nikhil Bhintade is a tech-savvy product manager and wordsmith, always eager to discover the latest and greatest in the tech world. With a sharp mind for all things technical, he is constantly exploring new ideas and looking for ways to push the boundaries of what's possible with technical content and products.
